...
public class Sample extends Applet {
	...
	public Sample() {
		...
		MagellanInputDevice magellanDevice = new MagellanInputDevice(magellan);
		magellanDevice.initialize();
		magellanDevice.setSensitivity(0.0001f);
		magellanDevice.setAngularRate(0.001f);
		u.getViewer().getPhysicalEnvironment().addInputDevice( magellanDevice );
			
		TransformGroup viewTrans = u.getViewingPlatform().getViewPlatformTransform();
		Sensor magellan1 = magellanDevice.getSensor(0);
		SensorBehavior s = new SensorBehavior(viewTrans, magellan1);
